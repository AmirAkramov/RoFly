...
LIBRARY=(OUT/'physiology-v6')/(
    'libmemory.dll' if sys.platform=='win32'
    else ('libmemory.dylib' if sys.platform=='darwin' else 'libmemory.so')
)
...
def build():
    ...
    if LIBRARY.exists() and metadata.exists():
        ...
        if record['source_sha256']==sha and record['binary_sha256']==hashlib.sha256(LIBRARY.read_bytes()).hexdigest():
            return record

    LIBRARY.parent.mkdir(parents=True,exist_ok=True)
    temp=LIBRARY.with_suffix(LIBRARY.suffix+'.partial')

    flags=['clang++','-O3','-std=c++17','-shared']

    if sys.platform!='win32':
        flags.append('-fPIC')

    flags.extend([str(SOURCE),'-o',str(temp)])
    ...
    record={
        ...
        'flags':['-O3','-std=c++17','-shared'] if sys.platform=='win32'
                else ['-O3','-std=c++17','-shared','-fPIC']
    }
