...
LIBRARY=(OUT/'physiology-v2')/('libmemory.dll' if sys.platform=='win32' else ('libmemory.dylib' if sys.platform=='darwin' else 'libmemory.so'))
...
def build():
    ...
    if sys.platform!='win32':flags.append('-fPIC')
    subprocess.run([*flags,str(SOURCE),'-o',str(temp)],check=True)
